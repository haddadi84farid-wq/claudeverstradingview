...
export async function setInputs({ entity_id, inputs: inputsRaw }) {
  ...
  const idJson = JSON.stringify(entity_id);
  const inputsJson = JSON.stringify(inputs);

  const result = await evaluate(`
    (function() {
      var chart = ${CHART_API};
      var study = chart.getStudyById(${idJson});
      if (!study) return { error: 'Study not found: ' + ${idJson} };
...
export async function toggleVisibility({ entity_id, visible }) {
  ...
  const result = await evaluate(`
    (function() {
      ...
      var study = chart.getStudyById(${idJson});
      if (!study) return { error: 'Study not found: ' + ${idJson} };
      study.setVisible(${visible ? 'true' : 'false'});
